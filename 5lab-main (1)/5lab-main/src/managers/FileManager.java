...
public class FileManager {
    ...
    public FileManager(String fileName, Scanner scanner) {
        this.fileName = fileName;
...
public void saveToFile(Stack<Dragon> dragons) {
    while (true) {
        try (FileWriter write = new FileWriter(fileName)) {
            ...
            try (FileOutputStream filer = new FileOutputStream("cfg.txt")) {
                filer.write(fileName.getBytes());
            } catch (IOException ex) {
                System.out.println("Ошибка сохранения пути");
            }
            break;
            ...
            if (name.isEmpty()) {
                System.out.println("Отказ от ввода");
                break;
            }
            fileName = name;
        }
